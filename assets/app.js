...
(() => {
  ...
  // Queensland Globe cannot be captured directly by this app: browser security
  // prevents a cross-origin portal being read into a canvas. Instead the assistant
  // opens the authoritative portal, accepts a screenshot from clipboard/file, and
  // prepares a report-ready image with a title, site coordinates and an optional
  // legend. The resulting JPEG is attached to the Queensland Globe source card,
  // so the existing persistence, report preview and all exports handle it normally.
  function openQldGlobeCapture(src) {
    if (!state.site || state.site.state !== "QLD") return;
    let sourceImage = null;
    const selected = new Set(["Imagery", "Map labels", "Road and rail"]);
    ...
    const empty = el("div", { class: "qg-empty" }, "Paste or choose a Queensland Globe screenshot to begin.");
    ...
    function loadFile(file) {
      ...
      const reader = new FileReader();
      reader.onload = () => { const img = new Image(); img.onload = () => { sourceImage = img; redraw(); }; img.onerror = () => toast("Could not read that image"); img.src = reader.result; };
      reader.readAsDataURL(file);
    }
    const fileInput = el("input", { type: "file", accept: "image/*", hidden: true, onchange: (e) => loadFile(e.target.files && e.target.files[0]) });
    const pasteZone = el("div", { class: "dropzone qg-drop", tabindex: "0" }, "📋 Paste a screenshot here (Ctrl/Cmd+V), drag it in, or ", el("button", { type: "button", class: "pick-btn", onclick: (e) => { e.stopPropagation(); fileInput.click(); } }, "choose an image"));
    pasteZone.addEventListener("click", () => pasteZone.focus());
    pasteZone.addEventListener("paste", (e) => { const files = filesFromClipboard(e); if (files.length) { e.preventDefault(); loadFile(files[0]); } });
    ...
    pasteZone.addEventListener("drop", (e) => { e.preventDefault(); pasteZone.classList.remove("drag"); loadFile(Array.from(e.dataTransfer.files || [])[0]); });
    ...
    const saveBtn = el("button", { type: "button", class: "btn primary", onclick: () => {
      if (!sourceImage) { toast("Paste or choose a screenshot first"); pasteZone.focus(); return; }
      redraw();
    ...
    dialog.append(
      ...
      el("div", { class: "qg-workflow" }, el("b", {}, "1. Open the Globe"), el("span", {}, "Coordinates are copied automatically. Turn on the relevant layers and frame the site, then use your device's screenshot tool."), el("a", { href: buildUrl(src), target: "_blank", rel: "noopener", class: "btn primary", onclick: () => copy(`${state.site.lat}, ${state.site.lon}`, "Coordinates copied") }, "Open Queensland Globe ↗")),
      el("div", { class: "qg-grid" },
        ...
        el("main", { class: "qg-editor" }, el("h3", {}, "2. Add and prepare the screenshot"), pasteZone, fileInput, el("label", { class: "field-label" }, "Map title", titleInput), el("div", { class: "qg-crop-head" }, el("b", {}, "Crop edges"), el("span", {}, "percentage of screenshot")), cropRow, el("label", { class: "inline-check" }, legendToggle, " Add selected-layer legend beside the map"), preview)),
    ...
    overlay.append(dialog); document.body.append(overlay); pasteZone.focus(); redraw();
  }
