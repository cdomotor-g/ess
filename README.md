...
3. Work down the **collection dashboard**. Each source opens aimed at the site.
   ...
   * At Queensland sites, the **Queensland Globe** card has a **Prepare map
     capture** workspace. Open the Globe with the coordinates copied, enable the
     relevant environmental layers, and paste or upload a screenshot. The
     workspace provides a searchable, colour-grouped layer legend, edge cropping,
     a report title and a live preview; saving attaches the prepared map to the
     card and carries it through the report, HTML, JSON and Print/PDF exports.
